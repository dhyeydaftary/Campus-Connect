    ...
    function buildSkeletonRows(cols, rows) {
        ...
        for (var r = 0; r < rows; r++) {
            ...
            for (var c = 0; c < cols; c++) {
                var w = widths[c] || 90;
                html += '<td><div class="skeleton-block" style="width:' + (w + (r * 15 % 30)) + 'px;"></div></td>';
            }
            html += '</tr>';
        }
        return html;
        ...
        fetch('/admin/api/tickets?status=' + mappedFilter)
            ...
            .then(function (data) {
                ...
                if (!data || data.length === 0) {
                    ticketsTableBody.innerHTML =
                        ...
                        '<p>There are no support tickets matching this filter.</p>' +
                        '</div></td></tr>';
                    updateStats([]);
                    return;
                }
                ...
                data.forEach(function (ticket, idx) {
                    ...
                    tr.innerHTML =
                        ...
                        '<td>' +
                        '<p class="tk-name">' + ticket.name + '</p>' +
                        '<p class="tk-email">' + ticket.email + '</p>' +
                        '</td>' +
                        '<td><span class="tk-subject" title="' + ticket.subject + '">' + ticket.subject + '</span></td>' +
                        '<td>' +
                        '<p class="tk-category">' + (ticket.category || '—') + '</p>' +
                        '<p style="margin-top:0.25rem;">' + getPriorityBadge(ticket.priority) + '</p>' +
                        '</td>' +
            ...
            .catch(function (err) {
                ...
                ticketsTableBody.innerHTML =
                    '<tr><td colspan="6">' +
                    '<div class="empty-state">' +
                    '<div class="empty-state-icon" style="background:hsl(0,90%,95%);color:hsl(0,65%,48%);"><i class="fa-solid fa-circle-exclamation"></i></div>' +
                    '<p style="color:hsl(0,65%,45%);">Error loading tickets</p>' +
        ...
        fetch('/admin/api/reports?status=' + mappedFilter)
            ...
            .then(function (data) {
                ...
                if (!data || data.length === 0) {
                    reportsTableBody.innerHTML =
                        ...
                        '<p>There are no issue reports matching this filter.</p>' +
                        '</div></td></tr>';
                    return;
                }
            ...
            .catch(function (err) {
                ...
                reportsTableBody.innerHTML =
                    ...
                    '<div class="empty-state">' +
                    '<div class="empty-state-icon" style="background:hsl(0,90%,95%);color:hsl(0,65%,48%);"><i class="fa-solid fa-circle-exclamation"></i></div>' +
                    '<p style="color:hsl(0,65%,45%);">Error loading reports</p>' +
                    '<p>Please try refreshing the page.</p>' +
                    '</div></td></tr>';
            });
    }
